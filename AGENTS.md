...
### Règles de calcul de l'émergence acoustique (CSP Art. R1336-7)

1. Variables d'entrée requises :
   - `bruit_ambiant` : niveau sonore total mesuré avec la nuisance, en dB(A).
   - `bruit_residuel` : niveau sonore habituel mesuré sans la nuisance, en dB(A).
   - `horaire` : heure du relevé au format HH:MM.
   - `duree_cumulee` : durée totale de la nuisance en secondes ou minutes.

2. Définition des paramètres :
   - **Période temporelle (`seuil_base`)** :
     - Si `horaire` est compris entre 07:00 et 21:59 -> période = DIURNE -> `seuil_base` = 5 dB(A)
     - Si `horaire` est compris entre 22:00 et 06:59 -> période = NOCTURNE -> `seuil_base` = 3 dB(A)
   - **Terme correctif (`correctif`) selon `duree_cumulee`** :
     - `duree_cumulee` <= 1 min (60 s) -> +6 dB(A)
     - 1 min < `duree_cumulee` <= 5 min -> +5 dB(A)
     - 5 min < `duree_cumulee` <= 20 min -> +4 dB(A)
     - 20 min < `duree_cumulee` <= 2 h (120 min) -> +3 dB(A)
     - 2 h < `duree_cumulee` <= 4 h (240 min) -> +2 dB(A)
     - 4 h < `duree_cumulee` <= 8 h (480 min) -> +1 dB(A)
     - `duree_cumulee` > 8 h -> +0 dB(A)
...
3. Algorithme d'évaluation :
   ...
   3. Évaluer la conformité :
      SI `emergence_mesuree <= emergence_limite` : CONFORME (Légal)
      SINON : NON_CONFORME (Infraction)

4. Règle de mesure spécifique (Contrainte de contrôle) :
   SI `duree_cumulee` < 10 secondes ALORS la durée d'enregistrement du `bruit_ambiant` doit obligatoirement être >= 10 secondes. Si ce n'est pas le cas, déclarer le relevé comme INVALIDE.
