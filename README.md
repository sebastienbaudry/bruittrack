...
La timeline du dashboard propose des fenêtres glissantes (1h/6h/24h/Tout) et un **zoom par brushing** : glisser sur la timeline sélectionne une plage temporelle qui verrouille l'axe ; double-clic ou Échap rétablit la fenêtre de boutons.
  **Zoom/dézoom axe Y (I61)** : molette sur le chronogramme = zoom fréquence SEUL, centré sur la position Y du curseur ; Ctrl+glisser vertical = translate fréquence ; double-clic, Échap ou clic sur le badge réinitialisent. Le zoom temps se fait par brushing (glisser horizontal).
